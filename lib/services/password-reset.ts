...
export async function requestReset(email: string, origin: string, ip: string | null) {
  ...
  if (user) {
    ...
    if (recent < MAX_REQUESTS_PER_HOUR) {
      ...
      await sendEmail({
        to: user.email,
        subject: "Reset your WrenchRates password",
        text:
          `Someone asked to reset the password for this WrenchRates account.\n\n` +
        ...
        html:
          `<p>Someone asked to reset the password for this WrenchRates account.</p>` +
